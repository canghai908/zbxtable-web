  ...
  <page-layout :noTitle="true">
    <a-form-model ref="ruleForm" :rules="rules" :model="form" :label-col="labelCol" :wrapper-col="wrapperCol">
      ...
      <a-form-model-item :label="$t('state')" prop="status">
        <a-switch :checked-children="$t('status_enabled')" :un-checked-children="$t('status_disabled')" v-model="form.status" />
      </a-form-model-item>
...
import { reportGetHosts, reportGetItems, reportGet, reportPut } from '@/services/admin'
      ...
      reportGet(id)
        .then((resp) => {
          ...
          if (res.code == 200) {
            ...
            this.form.hoststype = reportData.hoststype || ''
            this.form.emails = reportData.emails || ''
            this.form.desc = reportData.desc || ''
            this.form.status = reportData.status === '1' || reportData.status === 1
            this.form.cycle = reportData.cycle ? reportData.cycle.split(',') : ['day']
            this.form.report_type = 'host'
            ...
            if (reportData.host_ids) {
              try {
                ...
                if (Array.isArray(hostConfigs) && hostConfigs.length > 0) {
                  this.form.hostConfigs = hostConfigs.map(config => ({
                    host_id: config.host_id || '',
                    item_ids: config.item_ids || []
                  }))
                  
                  // 设置默认主机类型
                  if (this.form.hostConfigs.length > 0 && this.form.hostConfigs[0].host_id) {
                    // 需要根据host_id获取host类型，这里先使用已有的hoststype
                    if (this.form.hoststype) {
                      this.DefaultHostType = this.form.hoststype
                      this.handleChange(this.form.hoststype)
                    }
                  }
                }
              } catch (e) {
    ...
    onSubmit() {
      this.$refs.ruleForm.validate((valid) => {
        if (valid) {
          ...
          reportPut(this.id, params).then((resp) => {
            let res = resp.data
            if (res.code == 200) {
    ...
    handleChange(e) {
      ...
      reportGetHosts(params).then((resp) => {
        ...
        if (res.code == 200) {
          ...
          // 如果已有hostConfigs，加载对应的items
          this.form.hostConfigs.forEach((config, index) => {
            if (config.host_id) {
